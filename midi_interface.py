...
mw10 = MidiOutWrapper(midiout, ch=10)
mw11 = MidiOutWrapper(midiout, ch=11)
...
def handle_message(message, channel, q):
    if 'x' in message.message:
        if message.message['x'] != 0:
            if channel == 10:
                note, velocity = map_to(message.message['x'], message.message['y'], message.message['z'])
        ...
        if channel == 10:
            ...
            # time.sleep(1 + abs(message.message['z']))
            #mw11.send_note_off(note=note, velocity=velocity)
            q.put((datetime.datetime.now(), note))
            # mw11.send_note_off(note=note, velocity=velocity)
            # time.sleep(0.1)
        elif channel == 12:
            note, velocity = map_to_orientation(message.message['alpha'], message.message['beta'], message.message['gamma'])
...
if __name__ == '__main__':
    ...
    try:
            while True:
                mw = MidiOutWrapper(midiout, ch=3)
...
def clear_queue(q):
    while not q.empty():
        ...
        print(datetime.datetime.now() - item[0])
        print(q.qsize())
        global counter
        counter = counter + 1
        if item is not None and datetime.datetime.now() - item[0] > datetime.timedelta(seconds=SEC_TIME_TO_OFF):
            mw11.send_note_off(note=item[1])
            print('Set note off' + str(item[1]))
            counter = counter - 1
        else:
            break
